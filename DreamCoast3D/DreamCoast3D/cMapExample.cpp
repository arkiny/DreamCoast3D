...
void cMapExample::cPyramid::Setup(D3DXMATRIXA16& mat, D3DCOLOR c)
{
...
void cMapExample::cPyramid::Render()
{
...
void cMapExample::Setup(int nHalfTile, float fWidth){
	cGameMapObject::Setup();
	std::vector<ST_PC_VERTEX>	m_vecVertex;
...
void cMapExample::Update(float fDelta){
	cGameMapObject::Update(fDelta);
	// Do Nothing
}

void cMapExample::Render(){
	cGameMapObject::Render();
...
float cMapExample::GetHeight(OUT bool& isLand, IN D3DXVECTOR3* pvPosition){
	return cGameMapObject::GetHeight(isLand, pvPosition);
}